...
final class FileValidator
{
     ...
     * @throws FileException
     */
    public function validateForImport(): self
    {
        $this->mustExist();
        $this->mustBeFile();
        $this->mustBeReadable();
     ...
     * @throws FileException
     */
    public function validateForExport(bool $overwrite = false): self
    {
        $dir = dirname($this->filepath);

        if (!is_dir($dir)) {
            throw FileException::notFound($dir);
        }

        if (!is_writable($dir)) {
            throw FileException::notWritable($dir);
        }
